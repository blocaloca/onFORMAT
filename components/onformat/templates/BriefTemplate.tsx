...
export const BriefTemplate = ({ data, onUpdate, persona, isPrinting, plain, orientation, metadata }: {
    ...
    const inputStyle = "w-full bg-zinc-100 shadow-inner border border-zinc-200 rounded-md p-3 text-sm outline-none focus:border-zinc-400 focus:bg-white resize-none placeholder-zinc-300 min-h-[60px] font-sans";
    ...
    const renderField = (key: keyof BriefData, placeholder: string, minHeight: string = 'min-h-[60px]') => {
        ...
        return (
            <>
                ...
                />
                <div className={`${isPrinting ? 'block' : 'hidden print:block'} ${minHeight} w-full text-sm font-sans leading-normal text-black whitespace-pre-wrap bg-zinc-100 shadow-inner border border-zinc-200 rounded-md p-3 rounded-sm`}>
                    {textVal || "—"}
                </div>
            </>
        );
