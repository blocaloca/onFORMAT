...
interface MoodBoardItem {
    id: string; // concise unique id for key
    url: string;
    caption: string;
    aspectRatio: AspectRatio;
    size: ItemSize;
    showCaption?: boolean; // New optional toggle
    fullWidth?: boolean; // Deprecated, kept for migration check if needed
}
...
interface MoodBoardTemplateProps {
    data: Partial<MoodBoardData>;
    onUpdate: (data: Partial<MoodBoardData>) => void;
    isLocked?: boolean;
    plain?: boolean;
...
export const MoodBoardTemplate = ({ data, onUpdate, isLocked = false, plain, orientation, metadata, isPrinting }: MoodBoardTemplateProps) => {
    ...
    React.useEffect(() => {
        ...
        const newItems = items.map((item, idx) => {
            ...
            if (!anyItem.id || !anyItem.aspectRatio || !anyItem.size || anyItem.showCaption === undefined) {
                hasChanges = true;
                // Determine size from old fullWidth prop exists
                let initialSize: ItemSize = 'medium';
                if (anyItem.fullWidth) initialSize = 'large';

                return {
                    ...item,
                    id: anyItem.id || `item-${Date.now()}-${idx}`,
                    aspectRatio: anyItem.aspectRatio || '16:9',
                    size: anyItem.size || initialSize,
                    showCaption: anyItem.showCaption !== undefined ? anyItem.showCaption : true
                };
            }
            return item;
        });
        ...
        if (hasChanges) {
            onUpdate({ items: newItems as MoodBoardItem[] });
        }
    }, [data.items]); // Intentionally not including onUpdate to avoid potential infinite loops if reference unstable
    ...
    const handleAddItem = () => {
        const newItem: MoodBoardItem = {
            ...
            caption: '',
            aspectRatio: '16:9',
            size: 'medium', // Default to medium (half width)
            showCaption: true
        };
        onUpdate({ items: [...items, newItem] });
    };

    const handleUpdateItem = (index: number, updates: Partial<MoodBoardItem>) => {
    ...
    // Heuristic: Size score (col-span) corresponds to relative height for a given aspect ratio.
    // Small=2, Medium=3, Large=6.
    // MAX_PAGE_HEIGHT_SCORE represents the maximum accumulated "height units" for a page.
    // Landscape: 7 units (e.g., one Large item (6 units) + one Small item (2 units) would exceed,
    // or two Medium items (3+3=6 units) would fit, but a third Medium would exceed).
    // Portrait: 12 units (e.g., two Large items (6+6=12 units) would fit).
    const MAX_PAGE_HEIGHT_SCORE = orientation === 'landscape' ? 7 : 12;

    const pages: MoodBoardItem[][] = [];
    let currentPage: MoodBoardItem[] = [];
    let currentPageHeight = 0;

    // Temporary row tracking
    let currentRowItems: MoodBoardItem[] = [];
    ...
    const flushRowToPage = () => {
        ...
        // Determine row height score (max of items in row)
        // The tallest item in a grid row dictates the row's height.
        let rowHeight = 0;
        currentRowItems.forEach(i => {
            const h = getColSpan(i.size); // Col-span is used as a proxy for relative height
            if (h > rowHeight) rowHeight = h;
        });

        // Check if adding this row exceeds max page height.
        // If it does, and the current page already has items, push the current page and start a new one.
        ...
        // Add items from the flushed row to the current page
    ...
    (data.items || []).forEach((item) => {
        ...
        // If the current item doesn't fit in the current row (width-wise),
        // flush the current row to the page and start a new row.
    ...
    // Flush any remaining items in the last row to the current page
    flushRowToPage();

    // Push the very last page if it has content or if there are no items at all
            ...
            {pages.map((pageItems, pageIndex) => (
                ...
                >
                    <div className="space-y-6 h-full flex flex-col">
                        ...
                        {/* Overview Field - Only Page 1 */}
                        {pageIndex === 0 && (
                            <div>
                                <label className="block font-bold text-zinc-500 mb-1 text-[10px] uppercase tracking-widest">Overview</label>
                                <textarea
                                    disabled={isLocked}
                                    value={data.overview || ''}
                        ...
                        <div className="grid grid-cols-6 gap-6 flex-1 content-start">
                            {pageItems.map((item) => {
                                // We need to find the original index for update callbacks
                                const originalIndex = (data.items || []).findIndex(i => i.id === item.id);
                                return (
                                    <div
                                        key={item.id}
                                        className={`relative group flex flex-col gap-2 ${getSizeClass(item.size || 'medium')}`}
                                    >
                                        {/* Toolbar (Hover) */}
                                        {!isLocked && (
                                            <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white/90 backdrop-blur-sm rounded-md p-1 border border-zinc-200 shadow-sm">
                                                ...
                                                {/* Caption Toggle */}
                                                <div className="flex items-center border-r border-zinc-200 pr-1 mr-1 gap-1">
                                                    <button onClick={() => handleUpdateItem(originalIndex, { showCaption: !item.showCaption })} className={`p-1 rounded hover:bg-zinc-100 ${item.showCaption !== false ? 'text-black' : 'text-zinc-400'}`} title="Lines"><Type size={12} /></button>
                                                </div>

                                                {/* Delete Button with Confirmation Popover */}
                                                <div className="relative">
                                                    <button
                                                        onClick={() => setDeleteConfirmIndex(deleteConfirmIndex === originalIndex ? null : originalIndex)}
                                                        className={`p-1 rounded hover:bg-red-50 ml-1 hover:text-red-500 ${deleteConfirmIndex === originalIndex ? 'text-red-500' : 'text-zinc-400'}`}
                                                        title="Remove"
                                                    >
                                                        <Trash2 size={12} />
                                                    </button>
                        ...
                        {!isLocked && pageIndex === pages.length - 1 && (
                            <div className="py-4 flex justify-center print-hidden">
                                <button
                                    ...
                                    className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400 hover:text-black border border-dashed border-zinc-300 hover:border-black px-6 py-2 transition-colors rounded-sm"
                                >
                                    <span>+ Add Image</span>
                                </button>
                            </div>
