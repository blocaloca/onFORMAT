...
interface FloatingMobileControlProps {
    ...
    onMobileCheckIn?: (email: string) => void;
}

export const FloatingMobileControl = ({ data, onUpdate, onClose, metadata, crewList, userEmail, userRole, latestNotification, onMobileCheckIn }: FloatingMobileControlProps) => {
    ...
    useEffect(() => {
        ...
        if (isOwner) {
            ...
            if (onMobileCheckIn) onMobileCheckIn(userEmail);
            return;
        }
        ...
        const member = crewList.crew.find((m: any) => m.email?.toLowerCase().trim() === userEmail.toLowerCase().trim());
        ...
        >
            ...
            {accessDenied ? (
                <div className="p-12 flex flex-col items-center justify-center text-center h-[400px]">
                    ...
                    <h2 className="text-xl font-black uppercase tracking-wider text-rose-500 mb-4">Access Denied</h2>
                    <p className="text-zinc-400 text-sm mb-6 max-w-sm">
                        You are not listed in the Crew List for this project. <br />
                        Please contact the shoot producer.
                    </p>
                    <button onClick={onClose} className="px-6 py-2 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold uppercase tracking-widest rounded">
                        Close
                    </button>
                </div>
            ) : (
